...
export function getMinUnit(zoom, width, timeSteps) {
  ...
  let minUnit = 'year'; // this timespan is in ms initially
  ...
  Object.keys(timeDividers).some(unit => {
    ...
    const cellsToBeRenderedForCurrentUnit = nextTimeSpanInUnitContext / timeSteps[unit]; // what is happening here? why 3 if time steps are greater than 1??

    const cellWidthToUse = timeSteps[unit] && timeSteps[unit] > 1 ? 3 * minCellWidth : minCellWidth; // for the minWidth of a cell, how many cells would be rendered given
...
export function calculateDimensions({
  ...
  const itemTimeRange = itemTimeEnd - itemTimeStart; // restrict startTime and endTime to be bounded by canvasTimeStart and canvasTimeEnd
...
export function stackAll(itemsDimensions, groupOrders, lineHeight, stackItems) {
  ...
  for (var index in groupedItems) {
    ...
    const groupTop = sum(groupHeights); // Is group being stacked?

    const isGroupStacked = group.stackItems !== undefined ? group.stackItems : stackItems;
    const {
      groupHeight,
      verticalMargin
    } = stackGroup(itemsDimensions, isGroupStacked, lineHeight, groupTop); // If group height is overridden, push new height
...
export function stackTimelineItems(items, groups, canvasWidth, canvasTimeStart, canvasTimeEnd, keys, lineHeight, itemHeightRatio, stackItems, draggingItem, resizingItem, dragTime, resizingEdge, resizeTime, newGroupOrder) {
  ...
  const visibleItemsWithInteraction = visibleItems.map(item => getItemWithInteractions({
    item,
    keys,
    draggingItem,
    resizingItem,
  ...
  let dimensionItems = visibleItemsWithInteraction.map(item => getItemDimensions({
    item,
    keys,
    canvasTimeStart,
    canvasTimeEnd,
    ...
    lineHeight,
    itemHeightRatio
  })).filter(item => !!item); // Get a new array of groupOrders holding the stacked items

  const {
    height,
    groupHeights,
    groupTops
  } = stackAll(dimensionItems, groupOrders, lineHeight, stackItems);
  return {
    dimensionItems,
    height,
    groupHeights,
...
export function calculateScrollCanvas(visibleTimeStart, visibleTimeEnd, forceUpdateDimensions, items, groups, props, state) {
  ...
  const canKeepCanvas = newZoom === oldZoom && visibleTimeStart >= oldCanvasTimeStart + oldZoom * 0.5 && visibleTimeStart <= oldCanvasTimeStart + oldZoom * 1.5 && visibleTimeEnd >= oldCanvasTimeStart + oldZoom * 1.5 && visibleTimeEnd <= oldCanvasTimeStart + oldZoom * 2.5;
  ...
  if (!canKeepCanvas || forceUpdateDimensions) {
    ...
    const canvasWidth = getCanvasWidth(mergedState.width); // The canvas cannot be kept, so calculate the new items position
