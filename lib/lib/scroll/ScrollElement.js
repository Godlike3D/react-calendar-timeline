...
class ScrollElement extends Component {
  constructor() {
    ...
    _defineProperty(this, "refHandler", el => {
    ...
    _defineProperty(this, "handleWheel", e => {
      ...
      const {
        traditionalZoom
      } = this.props; // zoom in the time dimension

      if (e.ctrlKey || e.metaKey || e.altKey) {
        e.preventDefault();
        const parentPosition = getParentPosition(e.currentTarget);
        const xPosition = e.clientX - parentPosition.x;
        const speed = e.ctrlKey ? 10 : e.metaKey ? 3 : 1; // convert vertical zoom to horiziontal
        ...
        this.props.onScroll(this.scrollComponent.scrollLeft + (e.deltaY || e.deltaX)); // no modifier pressed? we prevented the default event, so scroll or zoom as needed
      }
    });
